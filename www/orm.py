...
'''
执行SELECT语句，我们用select函数执行，需要传入SQL语句和SQL参数
注意替换sql中的‘？’ 为MySQL中的‘%s’
size用于处理查询数量
'''
...
'''
由于insert,delete,update操作请求参数与返回相同。提供一个统一执行SQL的方法
'''
...
'''
    定义ModelMetaclass：
        1.排除对Model类的修改
        2.对当前的类的Field字段保存到__mapping__中，同时从类属性中删除该Field属性，否则，容易造成运行时错误（实例的属性会遮盖类的同名属性）
